...
namespace CoordinateSharp
{
    public partial class Cartesian
    {
        ...
        /// </code>
        /// </example>
        public static Cartesian Parse(string value)
        {
            Cartesian cart;
            if (TryParse(value, out cart)) { return cart; }
        ...
        /// </code>
        /// </example>
        public static bool TryParse(string value, out Cartesian cart)
        {
            double[] vals = null;
            ...
            if (FormatFinder.TryCartesian(value.ToUpper().Replace("KM", "").Replace("X", "").Replace("Y", "").Replace("Z", ""), out vals))
            {
                try
                {
                    cart = new Cartesian(vals[0], vals[1], vals[2]);
